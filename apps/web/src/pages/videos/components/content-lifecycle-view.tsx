...
interface ContentLifecycleViewProps {
  data: {
    trending: { id: string; platform: string; title: string; viewCount: number | null; region: string; fetchedAt: string } | null;
    download: { id: string; status: string; downloadedAt: string | null } | null;
    upload: { id: string; status: string; uploadedAt: string | null } | null;
    published: { id: string; platform: string; title: string; viewCount: number | null; likeCount: number | null; publishedAt: string | null };
  } | undefined;
}

const steps = [
  { key: 'trending', label: 'Trending Source', icon: TrendingUp, color: 'bg-orange-50 text-orange-700' },
...
export function ContentLifecycleView({ data }: ContentLifecycleViewProps) {
  ...
  const stepData: Record<string, { exists: boolean; lines: string[] }> = {
    trending: {
      ...
      lines: data.trending
        ? [`${data.trending.platform} - ${data.trending.region}`, `${formatCompactNumber(data.trending.viewCount)} views`, formatDate(data.trending.fetchedAt)]
        : ['No trending source'],
    },
    download: {
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div key={step.key} className="flex items-center gap-2">
                <div className={`min-w-[160px] rounded-lg border p-3 ${sd.exists ? '' : 'border-dashed opacity-50'}`}>
                  <div className="mb-2 flex items-center gap-2">
                    <div className={`rounded p-1 ${step.color}`}>
                      <Icon className="h-3 w-3" />
                    </div>
                    <span className="text-xs font-medium">{step.label}</span>
                    {sd.exists && <Badge variant="outline" className="ml-auto text-[10px]">Done</Badge>}
                  </div>
                  {sd.lines.map((line, j) => (
